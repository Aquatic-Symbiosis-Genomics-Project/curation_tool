...
HERE
  ...
  def build_pretext(y, fasta, out_file, highres = false)
    cmd = "/software/grit/projects/vgp_curation_scripts/Pretext_HiC_pipeline.sh -i #{fasta} -s #{out_file} -k #{y.hic_read_dir} -d `pwd` #{highres}"
    puts `#{cmd}`
    raise "something went wrong" unless $?.success?
  end
